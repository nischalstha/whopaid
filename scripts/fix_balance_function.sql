...
-- Create updated function that handles invited users with explicit column references
...
BEGIN
    ...
    WITH payments AS (
        ...
        SELECT 
            CASE 
                WHEN e.paid_by IN (SELECT tau.user_id FROM temp_all_users tau WHERE tau.is_invited = FALSE) THEN e.paid_by
