...
// SQL to create setup functions
const setupFunctionsSQL = `
...
-- Function to create the group_members table
CREATE OR REPLACE FUNCTION create_group_members_table()
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Function to create the expense_splits table
CREATE OR REPLACE FUNCTION create_expense_splits_table()
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  PERFORM create_groups_table();
  PERFORM create_group_members_table();
  PERFORM create_expenses_table();
  PERFORM create_expense_splits_table();
END;
$$;
...
-- Function to create the balance function
CREATE OR REPLACE FUNCTION create_balance_function()
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Function to create the user trigger
...
BEGIN
  ...
  PERFORM setup_rls_policies();
  PERFORM create_balance_function();
  PERFORM create_user_trigger();
END;
$$;

-- Grant execute permissions to authenticated users
GRANT EXECUTE ON FUNCTION create_users_table TO authenticated;
GRANT EXECUTE ON FUNCTION create_groups_table TO authenticated;
GRANT EXECUTE ON FUNCTION create_group_members_table TO authenticated;
GRANT EXECUTE ON FUNCTION create_expenses_table TO authenticated;
GRANT EXECUTE ON FUNCTION create_expense_splits_table TO authenticated;
GRANT EXECUTE ON FUNCTION create_tables TO authenticated;
GRANT EXECUTE ON FUNCTION setup_rls_policies TO authenticated;
GRANT EXECUTE ON FUNCTION create_balance_function TO authenticated;
GRANT EXECUTE ON FUNCTION create_user_trigger TO authenticated;
GRANT EXECUTE ON FUNCTION setup_database TO authenticated;
GRANT EXECUTE ON FUNCTION get_balances TO authenticated;
`

export async function GET() {
  return new Response(setupFunctionsSQL, {
    headers: {
...
export async function POST() {
  try {
    ...
    // We need to use the service role key to create functions
    // This is not possible from the client, so we'll return instructions
    return NextResponse.json({
      success: false,
      message: "Please run the SQL script in the Supabase SQL Editor",
      sql: setupFunctionsSQL,
    })
  } catch (error: any) {
